...
It all started when I received a mail from a HackerOne private program saying that a new domain was up and running. Given that the program itself is a very old and hardened one, I decided to give it a try since it could be a very good oportunity to hack it. I fired up Burp and started looking around.
...
This way, the default query that restricted access to other user's data gets bypassed, allowing the reatrieval of all the information available in the Solr endpoint. In the case of the `User` core, which contained all kinds of personal information, there were 796165 entries. Also, there were other cores mentioned before with critical data such as `UserLoginHistory`, `BusinessEntityAccount`, `EntityPayment`, `PasswordHistory`, etc. Here are some examples of the retrieved data:
...
Also, this vulnerability also allowed to list the files of the directories. For example, these are the contents of the root directory `/`:
